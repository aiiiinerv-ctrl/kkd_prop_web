 ...
 *   BACKUP_WRITES_QUIESCED    must be exactly "true" when any source table is
 *                             non-transactional; confirms the host layer has
 *                             blocked public and admin writes for the dump.
 *
...
const writesQuiesced = process.env.BACKUP_WRITES_QUIESCED === "true";
 ...
 */
async function buildDumpSql(db: PrismaClient): Promise<{ sql: string; counts: Record<string, number> }> {
  ...
  for (const { table, delegate } of BACKUP_MODELS) {
    ...
    const rows: Record<string, any>[] = await (db as any)[delegate].findMany();
...
async function main() {
  const sourcePreflight = await inspectSchema(prisma);
  if (!sourcePreflight.sourceTransactional && !writesQuiesced) {
    fail("source includes non-transactional tables; quiesce all writes and set BACKUP_WRITES_QUIESCED=true");
  }

  const { sql, counts, inspected } = await prisma.$transaction(
    async (tx) => {
      const dump = await buildDumpSql(tx as PrismaClient);
      const schema = await inspectSchema(tx, dump.counts);
  ...
  console.log(`✓ backup-db: snapshot ${path.basename(snapshotDir)} created`);
  console.log(`  - database.sql: ${formatBytes(dbSize)}, ${totalRows} row(s)`);
  console.log(`  - schema metadata: ${SCHEMA_METADATA_FILENAME} (${metadata.schemaSha256})`);
  console.log(`  - source transactional: ${metadata.sourceTransactional}`);
  console.log(`  - writes quiesced: ${metadata.writesQuiesced}`);
  for (const { table } of BACKUP_MODELS) {
    console.log(`      ${table}: ${counts[table]}`);
  }
  if (privateDestPath) {
    console.log(`  - storage/private: copied (${formatBytes(privateSize)})`);
  } else {
