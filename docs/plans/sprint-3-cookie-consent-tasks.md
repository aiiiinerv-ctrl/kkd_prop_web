...
6. **เปิด `/th/contact` ดูด้วยตา ทั้งสถานะ "ยังไม่ตอบแบนเนอร์" และ "กดปฏิเสธ"** — CookieYes auto-blocking อาจบล็อก iframe Google Maps ทำให้แผนที่กลายเป็นกล่องเปล่า แผนที่บนหน้าติดต่อเราเป็น trust signal ตรง ๆ ของผู้รับเหมาในพื้นที่ ("มีที่ตั้งจริงไหม") หายไปเงียบ ๆ กระทบ conversion มากกว่าเรื่อง cosmetic ทั้งหมดรวมกัน — ดู #27

**ของที่ต้องขอจากลูกค้า (ไม่ blocking การเปิดแบนเนอร์)**

- เนื้อหานโยบายความเป็นส่วนตัว + ข้อกำหนดการใช้งาน TH/EN — พอได้มาต้องแตะ **3 จุดพร้อมกัน**: หน้า policy, ลิงก์ footer, และ **บรรทัดใต้ปุ่ม submit ของฟอร์ม booking** ซึ่งเก็บชื่อ/เบอร์/ที่อยู่โดยยังไม่มีประกาศ ณ จุดเก็บ — จุดที่สามคือจุดที่หลุดง่ายที่สุด
- ที่อยู่จดทะเบียน + เลขทะเบียนนิติบุคคล 13 หลัก (ตอนนี้หน้า policy มีแค่ชื่อบริษัท + ช่องทางติดต่อ ซึ่งเพียงพอแล้วสำหรับข้อกำหนดว่าผู้ควบคุมข้อมูลต้องติดต่อได้)
